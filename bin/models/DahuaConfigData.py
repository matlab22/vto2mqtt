    ...
    def __init__(self, configfile: str):
        ...
        try:
            with open(self.configfile) as json_pcfg_file:
                ...
                hostConfig = dahuaConfigData['host']
                # configure host ip
                self.host = str(hostConfig['ip'])
                self.port = int(hostConfig.get('port', 5000))
                self.is_ssl = str(os.environ.get('DAHUA_VTO_SSL', False)).lower() == str(True).lower()

                self.username = str(hostConfig['username'])
                self.password = str(hostConfig['password'])

                self._auth = HTTPDigestAuth(self.username, self.password)
                self._base_url = f"{PROTOCOLS[self.is_ssl]}://{self.host}:{self.port}/cgi-bin/"
